                ...
                {hasSubInfo && (
                    <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5 mt-1 pl-[68px] text-[10px] text-gray-400 dark:text-gray-600">
                        {realInstallmentTotal > 0 && (
                            <span>💳 {formatCurrencyBRL(realInstallmentTotal)}</span>
                        )}
                        {realRecurringTotal > 0 && (
                        ...
                        {simulatedCreditTotal > 0 && (
                            <span style={{ color: 'var(--theme-accent)' }}>
                                +cred {formatCurrencyBRL(simulatedCreditTotal)}
                            </span>
                        )}
                        {simulatedDebitTotal > 0 && (
                            <span style={{ color: 'var(--theme-accent)' }}>
                                +déb {formatCurrencyBRL(simulatedDebitTotal)}
                            </span>
                        )}
                    </div>
                )}
                                ...
                                <>
                                    ...
                                    {recurringBreakdown.map((item) => (
                                        <div key={item.id} className="flex items-center justify-between">
                                            ...
                                            <span className="shrink-0 text-xs font-semibold ml-3 text-cyan-600 dark:text-cyan-400">
                                                {formatCurrencyBRL(item.amount)}
                                            </span>
                                        </div>
                                    ))}
...
export default function ProjectionMonthBar({ data, maxValue, index = 0 }) {
    ...
    const hasSubInfo   = realInstallmentTotal > 0 || realRecurringTotal > 0 || simulatedCreditTotal > 0 || simulatedDebitTotal > 0;
            ...
            >
                ...
                {hasSubInfo && (
                    <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5 mt-1 pl-[68px] text-[10px] text-gray-400 dark:text-gray-600">
                        ...
                        {realRecurringTotal > 0 && <span>🔁 {formatCurrencyBRL(realRecurringTotal)}</span>}
                        {simulatedCreditTotal > 0 && (
